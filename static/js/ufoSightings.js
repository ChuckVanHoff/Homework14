...
submit.on("click", function() {
  ...
  // Filter the data by getting the intersection of all the separate data sets
  var fullFilter = filteredData1.filter(x => filteredData2.includes(x))
                                .filter(x => filteredData3.includes(x))
                                .filter(x => filteredData4.includes(x))
                                .filter(x => filteredData5.includes(x));

  // For each sighting add a row to the table
  fullFilter.forEach((ufoData) => {
    var row = tbody.append("tr");
